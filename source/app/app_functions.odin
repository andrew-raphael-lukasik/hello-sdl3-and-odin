...
close :: proc ()
{
    ...
    when ODIN_DEBUG
    {
        for key, value in tracking_allocator.allocation_map { log.warnf("%v: Leaked %v bytes", value.location, value.size) }
        for value in tracking_allocator.bad_free_array { log.errorf("Bad free at: %v", value.location) }
    }
}
